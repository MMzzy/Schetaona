 import { useEffect, useRef, useState } from "react";
  ...
  export default function WalkersCarousel() {
    ...
    const getP = (start, end) => {
      ...
      return Math.min(1, Math.max(0, (scrollVal - start) / (end - start)));
    };

    const isMobile = window.innerWidth < 768;

    const titleOut  = isMobile ? easeOut(getP(0.08, 0.4))  :
  easeOut(getP(0.08, 0.25));
    const titleBack = isMobile ? easeOut(getP(0.67, 0.82)) :
  easeOut(getP(0.56, 0.75));
    const titleOpacity = Math.min(1, (1 - titleOut) + titleBack);
    const titleY = (1 - Math.max(1 - titleOut, titleBack)) * 50;
      ...
      <div ref={outerRef} data-nav-light className={styles.outer}>
        <div className={styles.sticky}>
          ...
          {walkers.map((walker, i) => {
            const spacing = isMobile ? 0.13 : 0.09;
            const start = 0.08 + i * spacing;
            const end = start + 0.85;
            const p = getP(start, end);
            const x = isMobile ? 120 - 360 * easeOut(p) : 110 - 190 *
  easeOut(p);
